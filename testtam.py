...
parser.add_argument("-n","--next", help="next passages to a stop")
parser.add_argument("-w", "--waiting", nargs = "*", help="time to wait")
...
c.execute(f"SELECT stop_name, route_short_name, trip_headsign, delay_sec FROM infoarret WHERE stop_name = '{args.next}' AND trip_headsign = '{args.waiting[0]}' AND route_short_name = '{args.waiting[1]}'")
