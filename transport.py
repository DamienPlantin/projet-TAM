...
def temps_arrive(horaire):
    ...
    strftime : allows to convert the variable time, which is in seconds ,
    in minutes and seconds
    """
    logger.debug("Appel de la fonction temps_arrive()")
    logger.debug("Conversion de %s sec en %s", horaire, time.strftime('%M min %S sec', time.gmtime(horaire)))
...
def download():
    ...
    urllib.request.urlretrieve : allows to upload the given file in the link
    and to rename it
    """
...
def clear_rows(cursor):
    ...
    This function allows to delete the table which is present
    in the main table 'infoarret'
    ...
    # Dans la parenthèse on peut faire une requête SQL


def insert_csv_row(csv_row, cursor):
    """Docstring of the function insert_csv_row

    This function allows to insert the informations of the csv file
    in the table 'infoarret'
    """
...
def remove_table(cursor):
    ...
    This function allows to delete the table 'infoarret'
    if one  existe in the data base
    """
...
def create_schema(cursor):
    ...
    This function allows to create a table 'infoarret'
    if it doesn't existe and to allocate the colonnes with their  type
    """
...
def next_passage(cursor):
    ...
    This function allows to informe about
    the next planned arrives in a given station
    """
...
def next_wait(c):
    ...
    This function allows to select the function next_passage
...
def time_wait(cursor):
    ...
    This function allows to pin up(display) the time waiting in a station ,
    for a line to a destination
    """
...
parser.add_argument("-u", "--update", help="update database with path file")
...
def main():
    ...
    This function allows to connect with the data base ,
    delete the table if it  existe.
    This function  allows also  to update the data base with a csv file
    ...
    # Connection entre sqlite et notre fichier db
    logger.debug("Connection/création du fichier tam.db pour la base de donnée")
    c = conn.cursor()
    # permet de se positionner sur la BdD
